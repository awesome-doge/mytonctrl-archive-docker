...
RUN wget -nv https://raw.githubusercontent.com/gdraheim/docker-systemctl-replacement/master/files/docker/systemctl3.py -O /usr/bin/systemctl  \
    ...
    && sed -i 's/User=validator/User=root/g; s/Group=validator/Group=root/g' /etc/systemd/system/validator.service \
    ...
    && sed -i 's/--threads [0-9]\+/--threads 16/' /etc/systemd/system/validator.service \
    && sed -i '/ExecStart/s/--threads [0-9]\+/--threads 16/' /etc/systemd/system/validator.service \
